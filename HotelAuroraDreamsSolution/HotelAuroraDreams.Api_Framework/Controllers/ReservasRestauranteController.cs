...
namespace HotelAuroraDreams.Api_Framework.Controllers
{
    ...
    public class ReservasRestauranteController : ApiController
    {
        ...
        // POST: api/ReservasRestaurante/Disponibilidad
        [HttpPost]
        ...
        public async Task<IHttpActionResult> VerificarDisponibilidadRestaurante(RestauranteDisponibilidadRequestDto requestDto)
        {
            ...
            try
            {
                ...
                {
                    return Ok(new RestauranteDisponibleViewModel { RestauranteID = restaurante.restaurante_id, NombreRestaurante = restaurante.nombre, Disponible = false, Mensaje = "Restaurante no tiene capacidad definida o no acepta reservas." });
                }

                // Lógica de disponibilidad: Sumar comensales de reservas confirmadas/atendidas para esa fecha y una ventana de tiempo
                // Por ejemplo, +/- 1 hora alrededor de la hora solicitada.
                TimeSpan ventanaAntes = requestDto.Hora.Subtract(TimeSpan.FromHours(1));
                TimeSpan ventanaDespues = requestDto.Hora.Add(TimeSpan.FromHours(1)); // Asumiendo una duración promedio de 1h para estar ocupado
                ...
                int comensalesReservados = await db.Reserva_Restaurante
                    .Where(rr => rr.restaurante_id == requestDto.RestauranteID &&
                                 ...
                                 (rr.hora_reserva >= ventanaAntes && rr.hora_reserva <= ventanaDespues) // Simplificación de solapamiento
                                 )
                ...
                {
                    return Ok(new RestauranteDisponibleViewModel { RestauranteID = restaurante.restaurante_id, NombreRestaurante = restaurante.nombre, Disponible = true, Mensaje = "Restaurante disponible.", CapacidadRestanteEstimada = capacidadRestante - requestDto.NumeroComensales });
                }
                else
                {
                    return Ok(new RestauranteDisponibleViewModel { RestauranteID = restaurante.restaurante_id, NombreRestaurante = restaurante.nombre, Disponible = false, Mensaje = $"No hay suficiente capacidad. Capacidad restante estimada: {Math.Max(0, capacidadRestante)} comensales.", CapacidadRestanteEstimada = Math.Max(0, capacidadRestante) });
                }
            }
            catch (Exception ex)
            {
