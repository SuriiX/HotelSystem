...
namespace HotelAuroraDreams.Api_Framework.Controllers
{
    ...
    public class FacturasController : ApiController
    {
        ...
        [HttpGet]
        [Route("")]
        public async Task<IHttpActionResult> GetFacturas([FromUri] int? clienteId = null, [FromUri] int? reservaId = null, [FromUri] DateTime? fechaDesde = null, [FromUri] DateTime? fechaHasta = null, [FromUri] string estado = null)
        {
            try
            {
            ...
            {
                return InternalServerError(new Exception($"Error al obtener facturas: {ex.ToString()}"));
            }
        }
        ...
        public async Task<IHttpActionResult> GetFactura(int id)
        {
            ...
            {
                return InternalServerError(new Exception($"Error al obtener factura ID {id}: {ex.ToString()}"));
            }
        }
        ...
        public async Task<HttpResponseMessage> GetFacturaPdf(int id)
        {
            try
            {
                ...
                using (MemoryStream ms = new MemoryStream())
                {
                    ...
                    {
                        Font smallFont1 = smallFont;
                        detailsTable.AddCell(new PdfPCell(new Phrase(detalle.descripcion_concepto, smallFont1)) { Padding = 5 });
            ...
            {
                return Request.CreateErrorResponse(HttpStatusCode.InternalServerError, $"Error al generar PDF de factura: {ex.ToString()}");
            }
        }
